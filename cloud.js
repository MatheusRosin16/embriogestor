/* ============================================================
   EmbrioGestor v2.1 - Google Drive / sincronização de segurança
   ============================================================ */
...
function cloudStatus(texto,tipo="info"){
  ...
  el.className=`cloud-status ${tipo}`; el.textContent=texto;
}
...
function carregarGoogleIdentity(){
  ...
  driveScriptPromise=new Promise((resolve,reject)=>{
    ...
    s.onload=()=>resolve(); s.onerror=()=>reject(new Error("Não foi possível carregar o Google Identity Services."));
...
async function conectarGoogleDrive(){
  if(!cloudConfigured()){
    cloudStatus("Falta configurar o GOOGLE_CLIENT_ID no arquivo config.js.","erro"); return false;
  }
  ...
  try{
    ...
    return await new Promise((resolve)=>{
      driveTokenClient=google.accounts.oauth2.initTokenClient({
        ...
        callback:(resp)=>{
          if(resp?.access_token){ driveToken=resp.access_token; driveFolderId=null; cloudStatus("Google Drive conectado.","ok"); paginaNuvem(); resolve(true); }
          else { cloudStatus("A conexão com o Google Drive não foi concluída.","erro"); resolve(false); }
        }
      });
      driveTokenClient.requestAccessToken({prompt:driveToken?"":"consent"});
    });
  }catch(e){ console.error(e); cloudStatus(e.message||"Erro ao conectar ao Google Drive.","erro"); return false; }
}

async function driveFetch(url,options={}){
  if(!driveToken){ const ok=await conectarGoogleDrive(); if(!ok) throw new Error("Google Drive não conectado."); }
  const headers=new Headers(options.headers||{}); headers.set("Authorization",`Bearer ${driveToken}`);
  const r=await fetch(url,{...options,headers});
  if(r.status===401){ driveToken=null; throw new Error("A sessão do Google expirou. Conecte novamente."); }
  if(!r.ok){ let t=""; try{t=await r.text()}catch{} throw new Error(`Google Drive: ${r.status} ${t}`.slice(0,350)); }
  return r;
}
...
async function obterPastaDrive(){
  ...
  const created=await (await driveFetch("https://www.googleapis.com/drive/v3/files?fields=id,name",{
    method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:nome,mimeType:"application/vnd.google-apps.folder"})
  })).json();
  driveFolderId=created.id; return driveFolderId;
}

function pacoteBackup(){
  return { formato:"EmbrioGestorBackup", versaoPacote:1, exportadoEm:new Date().toISOString(), banco:db };
}
...
async function uploadMultipartDrive(nome,conteudo,mime="application/json",pastaId=null){
  ...
  const meta={name:nome}; if(pastaId) meta.parents=[pastaId];
  const blob=new Blob([
...
async function salvarBackupDrive(silencioso=false){
  ...
  try{
    ...
    if(!silencioso){ cloudStatus(`Backup salvo no Drive: ${file.name}`,"ok"); await listarBackupsDrive(); }
  }catch(e){ console.error(e); if(!silencioso)cloudStatus(e.message||"Falha ao salvar backup.","erro"); }
...
async function buscarArquivoPorNome(nome){
  ...
  const data=await (await driveFetch(u)).json(); return data.files||[];
}
  ...
  try{
    ...
    if(achados.length){
      await driveFetch(`https://www.googleapis.com/upload/drive/v3/files/${achados[0].id}?uploadType=media`,{
        method:"PATCH",headers:{"Content-Type":"application/json"},body:txt
      });
    } else {
    ...
    cloudStatus("Dados principais sincronizados com o Google Drive.","ok");
    paginaNuvem();
  }catch(e){ console.error(e); cloudStatus(e.message||"Falha na sincronização.","erro"); }
  finally{ cloudBusy=false; }
}
...
async function carregarDadosPrincipaisDrive(){
  if(cloudBusy)return; cloudBusy=true;
  try{
    if(!confirm("Carregar os dados do Drive substituirá os dados atuais deste aparelho. Deseja continuar?")) return;
    cloudStatus("Carregando dados principais do Drive...","info");
    const nome=CLOUD_CFG.DRIVE_MASTER_FILE||"EmbrioGestor_Dados_Principais.json";
    const achados=await buscarArquivoPorNome(nome);
    if(!achados.length) throw new Error("Ainda não existe um arquivo principal no Drive.");
    const data=await (await driveFetch(`https://www.googleapis.com/drive/v3/files/${achados[0].id}?alt=media`)).json();
    aplicarBackupCloud(data);
    cloudStatus("Dados do Drive carregados com sucesso.","ok");
  }catch(e){ console.error(e); cloudStatus(e.message||"Falha ao carregar dados.","erro"); }
  finally{ cloudBusy=false; }
}
...
async function listarBackupsDrive(){
  const lista=document.getElementById("cloudBackupList"); if(lista)lista.innerHTML="<p>Carregando backups...</p>";
  try{
    const pasta=await obterPastaDrive(); const prefix=CLOUD_CFG.BACKUP_PREFIX||"EmbrioGestor_Backup_";
    ...
    const data=await (await driveFetch(u)).json();
    if(!lista)return;
    if(!data.files?.length){lista.innerHTML="<p>Nenhum backup encontrado no Drive.</p>";return;}
    lista.innerHTML=`<div class="cloud-backup-table">${data.files.map(f=>`<div class="cloud-backup-row"><div><b>${cloudEsc(f.name)}</b><small>${new Date(f.modifiedTime).toLocaleString("pt-BR")}</small></div><button class="btn secondary" onclick="restaurarBackupDrive('${f.id}')">Restaurar</button></div>`).join("")}</div>`;
  }catch(e){ if(lista)lista.innerHTML=`<p class="text-danger">${cloudEsc(e.message)}</p>`; }
}
...
async function restaurarBackupDrive(fileId){
  try{
    ...
    const data=await (await driveFetch(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`)).json();
    aplicarBackupCloud(data); alert("Backup restaurado com sucesso.");
  }catch(e){ alert(e.message||"Falha ao restaurar backup."); }
}
...
if(typeof salvarBanco==="function"){
  ...
  salvarBanco=function(){ _salvarBancoLocal(); agendarAutoBackupDrive(); };
}

function paginaNuvem(){
  header("Nuvem & Backup","Google Drive, segurança e sincronização entre dispositivos");
  ...
  c.innerHTML=`
    <div class="cloud-grid">
      <section class="card">
        ...
        <label class="cloud-toggle"><input type="checkbox" ${autoBackupAtivo()?"checked":""} onchange="setAutoBackup(this.checked)"> Fazer backup automático após alterações, quando o Drive estiver conectado</label>
        ${!configurado?`<div class="cloud-help"><b>Para ativar:</b> abra <code>config.js</code> e preencha <code>GOOGLE_CLIENT_ID</code>. O aplicativo precisa estar publicado em HTTPS para o login do Google funcionar corretamente.</div>`:""}
      </section>
      <section class="card">
        ...
        <div class="cloud-actions vertical">
          <button class="btn primary" onclick="salvarDadosPrincipaisDrive()" ${driveToken?"":"disabled"}>Enviar dados deste aparelho</button>
          <button class="btn secondary" onclick="carregarDadosPrincipaisDrive()" ${driveToken?"":"disabled"}>Carregar dados do Drive</button>
        </div>
    ...
    <section class="card">
      ...
      <p class="muted">A versão atual continua gravando imediatamente no aparelho e pode sincronizar o arquivo principal pelo Drive. A camada de nuvem foi separada do restante do sistema para permitir a próxima etapa: banco online com login e edição simultânea sem alterar os cadastros atuais.</p>
      <div id="cloudStatus" class="cloud-status info">${driveToken?"Google Drive conectado e pronto.":"Os dados locais continuam protegidos neste aparelho. Conecte o Drive para criar cópias externas."}</div>
    </section>`;
}
