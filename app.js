...
function formMovimentacao(id="") {
  ...
  ${select("Tipo","tipo",["Uso","Devolução","Ajuste positivo","Ajuste negativo"],m.tipo||"")}${campo("Quantidade","quantidade",m.quantidade??"1","number")}${campo("Data","data",m.data||hoje(),"date")}${campo("Observação","obs",m.obs||"")}
...
function formEstoque(id="",clienteId=""){
  ...
  modal(id?"Editar estoque de sêmen":"Nova entrada de sêmen",`<div class="form-grid">${select("Cliente","clienteId",clientesDisponiveisProfissional(),clienteId)}${select("Touro","touroId",db.touros,e.touroId||"")}${campo("Partida / lote","partida",e.partida||"")}${campo("Quantidade de entrada","quantidade",e.quantidade??0,"number")}${campo("Data de entrada","data",e.data||hoje(),"date")}<div><label>Tipo de recipiente</label><input value="CANECA" disabled><input type="hidden" name="recipienteTipo" value="CANECA"></div>${campo("Identificação da caneca","recipiente",e.recipiente||"")}${campo("Observações","obs",e.obs||"")}</div><br><button class="btn" onclick="salvarEstoque('${id}')">Salvar</button>`);
}



// ============================================================
// V2.3 — USUÁRIOS, PERFIS DE ACESSO E ESCOPO POR PROFISSIONAL
// ============================================================

const EG_PERFIS = ["Administrador","Veterinário","Laboratório","Consulta"];
const EG_MENU_PERFIL = {
  "Administrador": ["dashboard","clientes","doadoras","touros","racas","profissionais","carteiras","estoque","estoqueEmbrioes","movimentacoes","aspiracoes","producoes","transferencias","relatorios","usuarios","nuvem"],
  "Veterinário": ["dashboard","clientes","doadoras","touros","racas","estoque","estoqueEmbrioes","movimentacoes","aspiracoes","producoes","transferencias","relatorios"],
  "Laboratório": ["dashboard","clientes","doadoras","touros","racas","estoque","estoqueEmbrioes","movimentacoes","aspiracoes","producoes","transferencias","relatorios"],
  "Consulta": ["dashboard","clientes","doadoras","touros","racas","estoque","estoqueEmbrioes","aspiracoes","producoes","transferencias","relatorios"]
};
...
function egProfissionalSessaoId(){return egSessaoApp()?.profissionalId||"";}
function egEhAdmin(){return egPerfilAtual()==="Administrador";}
function egSomenteConsulta(){return egPerfilAtual()==="Consulta";}
function egPaginaPermitida(id){return (EG_MENU_PERFIL[egPerfilAtual()]||EG_MENU_PERFIL.Consulta).includes(id);}
function egPodeEditar(){return !egSomenteConsulta();}
function egNegarEdicao(){alert("Seu perfil é somente Consulta. Este usuário não pode criar, editar ou excluir registros.");return false;}
...
function egAplicarMenuAcesso(){
  ...
  if(rodape){const s=egSessaoApp();rodape.innerHTML=s?`<strong>${esc(s.nome||s.usuario||'Usuário')}</strong><br>${esc(s.perfil||'')}<br><small>Dados locais + Google Drive</small>`:'Dados locais + Google Drive opcional';}
}

// A navegação também valida a permissão; esconder o menu não é a única proteção.
function irPara(novaPagina){
  if(!egPaginaPermitida(novaPagina)){alert("Seu perfil não possui acesso a este módulo.");return;}
  page=novaPagina; clienteSelecionado=null; render();
}

// Para Administrador continua existindo o seletor manual de carteira.
// Para os demais perfis, o filtro é sempre o profissional vinculado ao usuário.
function clientesDisponiveisProfissional(){
  if(egEhAdmin()){
    if(!profissionalAtivoId)return db.clientes||[];
    return (db.clientes||[]).filter(c=>c.profissionalId===profissionalAtivoId);
  }
  const pid=egProfissionalSessaoId();
  if(!pid)return [];
  return (db.clientes||[]).filter(c=>c.profissionalId===pid);
}
...
function profissionalAtivoNome(){
  if(!egEhAdmin())return profissionalNome(egProfissionalSessaoId())||"Sem profissional vinculado";
...
function contextoProfissionalHTML(){
  ...
  return `<div class="global-filter-banner">Carteira visível: <strong>${esc(nome)}</strong>${egEhAdmin()&&profissionalAtivoId?` <button class="btn small secondary" onclick="limparProfissionalAtivo()">Ver todos</button>`:""}</div>`;
}
...
// Páginas que historicamente listavam a base inteira passam pelo escopo do usuário.
...
// Carteiras e Nuvem são administrativas.
...
// Cliente: não-admin fica obrigatoriamente vinculado ao próprio profissional.
function formCliente(id=""){
  if(!egPodeEditar())return egNegarEdicao();
  const c=db.clientes.find(x=>x.id===id)||{};
  if(id&&!egEhAdmin()&&!clientePermitidoNoContexto(id)){alert("Cliente fora da sua carteira.");return;}
  const pid=egEhAdmin()?(c.profissionalId||profissionalAtivoId||""):egProfissionalSessaoId();
  const campoProf=egEhAdmin()?select("Profissional responsável","profissionalId",db.profissionais,pid):`<div><label>Profissional responsável</label><input value="${esc(profissionalNome(pid)||'Não vinculado')}" disabled><input type="hidden" name="profissionalId" value="${esc(pid)}"></div>`;
...
function salvarCliente(id=""){
  ...
  const get=n=>document.querySelector(`[name="${n}"]`)?.value.trim()||"";
  if(!exigir(get("nome"),"Informe o nome do cliente."))return;
  const pid=egEhAdmin()?get("profissionalId"):egProfissionalSessaoId();
  if(!egEhAdmin()&&!pid){alert("Seu usuário não está vinculado a um profissional.");return;}
  const obj={id:id||idNovo("CLI",db.clientes),nome:get("nome"),cpf:get("cpf"),propriedade:get("propriedade"),municipio:get("municipio"),uf:get("uf"),telefone:get("telefone"),email:get("email"),profissionalId:pid};
  if(id){const a=db.clientes.find(x=>x.id===id);if(!a||(!egEhAdmin()&&!clientePermitidoNoContexto(id))){alert("Cliente fora da sua carteira.");return;}Object.assign(a,obj);}else db.clientes.push(obj);
  fecharModal();salvarBanco();
}

// Usuários ficam dentro do banco e, portanto, acompanham backup/sincronização do Drive.
function usuariosPermissoes(){
  if(!egEhAdmin()){alert("Somente Administradores podem gerenciar usuários.");page="dashboard";return dashboard();}
  header("Usuários e Permissões","Perfis de acesso e vínculo com profissionais");
  if(!Array.isArray(db.usuarios))db.usuarios=[];
  const linhas=db.usuarios.map(u=>`<tr><td>${esc(u.nome||"")}</td><td>${esc(u.usuario||"")}</td><td>${badge(u.perfil||"Consulta")}</td><td>${esc(profissionalNome(u.profissionalId)||"—")}</td><td>${badge(u.ativo===false?"Inativo":"Ativo")}</td><td><button class="btn small secondary" onclick="formUsuario('${u.id}')">Editar</button><button class="btn small danger" onclick="excluirUsuario('${u.id}')">Excluir</button></td></tr>`).join("");
  document.getElementById("content").innerHTML=`<div class="card"><div class="section-title"><div><h3>Usuários</h3><p class="muted">Administrador vê tudo. Veterinário, Laboratório e Consulta enxergam somente clientes do profissional vinculado.</p></div><button class="btn" onclick="formUsuario()">Novo usuário</button></div><div class="table-wrap"><table><thead><tr><th>Nome</th><th>Usuário</th><th>Perfil</th><th>Profissional vinculado</th><th>Status</th><th>Ações</th></tr></thead><tbody>${linhas||'<tr><td colspan="6">Nenhum usuário cadastrado.</td></tr>'}</tbody></table></div></div>`;
}
function formUsuario(id=""){
  if(!egEhAdmin())return;
  const u=(db.usuarios||[]).find(x=>x.id===id)||{};
  modal(id?"Editar usuário":"Novo usuário",`<div class="form-grid">${campo("Nome","nome",u.nome||"")}${campo("Usuário","usuario",u.usuario||"")}${select("Perfil","perfil",EG_PERFIS,u.perfil||"Consulta")}${select("Profissional vinculado","profissionalId",db.profissionais,u.profissionalId||"")}<div><label>${id?"Nova senha (deixe em branco para manter)":"Senha"}</label><input name="senha" type="password" autocomplete="new-password" placeholder="Mínimo 6 caracteres"></div>${select("Status","ativo",["Ativo","Inativo"],u.ativo===false?"Inativo":"Ativo")}</div><br><button class="btn" onclick="salvarUsuarioSistema('${id}')">Salvar usuário</button>`);
}
async function salvarUsuarioSistema(id=""){
  if(!egEhAdmin())return;
  const get=n=>document.querySelector(`[name="${n}"]`)?.value.trim()||"";
  const nome=get("nome"),usuario=get("usuario").toLowerCase(),perfil=get("perfil")||"Consulta",pid=get("profissionalId"),senha=document.querySelector('[name="senha"]')?.value||"";
  if(!nome||!usuario){alert("Informe nome e usuário.");return;}
  if(!EG_PERFIS.includes(perfil)){alert("Perfil inválido.");return;}
  if(perfil!=="Administrador"&&!pid){alert("Vincule um profissional para este perfil.");return;}
  ...
  const obj={id:id||idNovo("USR",db.usuarios),nome,usuario,perfil,profissionalId:perfil==="Administrador"?"":pid,senhaHash,ativo:get("ativo")!=="Inativo",criadoEm:atual?.criadoEm||new Date().toISOString(),atualizadoEm:new Date().toISOString()};
...
function excluirUsuario(id){
  ...
  if(!confirmarExclusao(`Excluir o usuário ${u.nome||u.usuario}?`))return;
  db.usuarios=db.usuarios.filter(x=>x.id!==id);salvarBanco();
}

// Consulta: bloqueio efetivo das ações de alteração, mesmo se um botão antigo ainda estiver visível.
if(!window.__EG_MUTACOES_PROTEGIDAS__){
  window.__EG_MUTACOES_PROTEGIDAS__=true;
  ["salvarDoadora","salvarTouro","salvarRaca","salvarProfissional","salvarEstoque","salvarMovimentacao","salvarAspiracao","salvarProducao","salvarTransferencia","salvarDiagnostico","salvarEstoqueEmbriao","salvarDoseServico",
...
function egAtualizarInterfacePerfil(){
  if(!egEhAdmin()) profissionalAtivoId=egProfissionalSessaoId();
  egAplicarMenuAcesso();
