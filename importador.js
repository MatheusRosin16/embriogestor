...
function impRacaNome(v){
  ...
  const mapa={HO:"Holandês",HOL:"Holandês",JE:"Jersey",AN:"Aberdeen Angus",HH:"Hereford",BO:"Braford",BF:"Braford",BN:"Brangus",BR:"Brangus",NE:"Nelore",GI:"Gir",GL:"Gir Leiteiro",SI:"Simental",SE:"Senepol",GU:"Guzerá",CA:"Canchim",CH:"Charolês",LI:"Limousin",RA:"Red Angus"};
...
function impDoadora(clienteId,nome,raca=""){
  nome=String(nome??"").trim(); if(!nome)return null;
  let d=(db.doadoras||[]).find(x=>x.clienteId===clienteId&&impNorm(x.nome)===impNorm(nome));
  if(d){if(!d.raca&&raca)d.raca=impRacaNome(raca);return d;}
  const rn=impRacaNome(raca)||"Outra"; if(rn&&!db.racas.includes(rn))db.racas.push(rn);
  d={id:idNovo("DOA",db.doadoras),clienteId,nome,registro:"",raca:rn,categoria:"Vaca",nascimento:"",status:"Ativo",obs:"Importada de planilha histórica"};
...
function paginaImportarDados(){
  ...
  header("Importar Dados Antigos","Excel: estoque de sêmen, histórico PIVE e estoque de embriões ABS");
  document.getElementById("content").innerHTML=`<div class="card"><h3>Importar planilha Excel</h3><p class="muted">O importador <b>mescla</b> os registros com a base atual. Ele não apaga seus clientes, produções ou usuários. Registros já importados do mesmo arquivo/linha são ignorados.</p><input id="impArquivo" type="file" accept=".xlsx,.xls" onchange="impLerArquivo(this)"><div id="impResultado" style="margin-top:16px"></div><div class="cloud-warning"><b>Antes de importar:</b> mantenha um backup no Google Drive. Quantidades de sêmen fracionadas (ex.: 2,5 doses) são aceitas. Valores suspeitos, principalmente células do Excel interpretadas como datas, são sinalizados para revisão.</div></div>`;
}
